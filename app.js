...
function switchView(viewId) {
    try {
        ...
        if (viewId === "dashboard") {
            if (title) title.innerText = "Controle Corporativo de POPs";
    ...
    try {
        ...
        paginatedItems.forEach(pop => {
            ...
            if (pop.status === "REVISADO") {
                ...
                statusIcon = "fa-stamp";
            } else if (pop.status === "AGUARDANDO APROVAÇAO" || pop.status === "AGUARDANDO REVISÃO") {
            ...
            let approveBtn = "";
            if (currentUser.isAdmin && (pop.status === "AGUARDANDO APROVAÇAO" || pop.status === "AGUARDANDO REVISÃO")) {
...
let currentPDFExportType = 'TRAININGS';
